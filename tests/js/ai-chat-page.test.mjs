...
test('AI interaction lives in the dedicated chat page instead of the dashboard panel', () => {
    ...
    assert.match(chatPage, /import \{ computed, nextTick, onBeforeUnmount, reactive, ref, watch \} from ['"]vue['"]/)
    assert.match(chatPage, /import \{ Head, router, setLayoutProps \} from ['"]@inertiajs\/vue3['"]/)
    assert.match(chatPage, /Single-lane chat with session memory, tool calls, and streamed output\./)
    ...
    assert.match(chatPage, /const effectiveArtifactMode = computed\(\(\) => \(aiDebugEnabled \? artifactMode\.value : 'auto'\)\)/)
    assert.match(chatPage, /import AiArtifactRenderer from ['"]@\/components\/ai\/AiArtifactRenderer\.vue['"]/)
    ...
    assert.match(chatPage, /setLayoutProps\(\{\s*mainClass: 'h-\[calc\(100dvh-3\.5rem\)\] overflow-hidden'/)
    assert.match(chatPage, /contentClass: 'h-full p-0'/)
    assert.match(chatPage, /class="grid h-full min-h-0 flex-1 lg:grid-cols-\[19rem_minmax\(0,1fr\)\]"/)
    ...
    assert.doesNotMatch(chatPage, /import PageWidth from ['"]@\/components\/layout\/PageWidth\.vue['"]/)
